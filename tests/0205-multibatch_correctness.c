...
static rd_kafka_t *create_verify_consumer_0205(const char *topic,
                                               const char *group_id,
                                               int part_cnt) {
        rd_kafka_conf_t *conf;
        rd_kafka_t *rk;
        ...
        test_consumer_assign("0205.assign", rk, parts);
...
static void dump_full_mismatch_report(verify_state_t *vs) {
        ...
        TEST_SAY("========== END 0205 FULL MISMATCH REPORT ==========\n");
}
...
static void run_consume_and_verify_phase(bench_config_t *config,
        ...
        rd_kafka_t *rk =
            create_verify_consumer_0205(topic, group_id, config->partition_cnt);
...
int main_0205_multibatch_correctness(int argc, char **argv) {
